    ...
    return (
      <>
        <div className='header'>
          <button onClick={this.prevMonth} className="button">{"previous"}</button>
          {this.months[this.state.date.getMonth()]}
          {this.state.date.getFullYear()}
          <button onClick={this.nextMonth} className="button">{"next"}</button></div>
      </>
    );
  }
}
    ...
    for (let i = 0; i < selectedDays.length; i++) {
      if (
        ...
        if (selectedDays[i].getDate() !== currentDate.getDate())
          return { border: "2px solid #660000" };
      }
    }
    if (
